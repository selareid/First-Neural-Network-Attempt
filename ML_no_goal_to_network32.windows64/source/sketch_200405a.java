...
class Network {
  ...
  Network () {
    ...
    layers[2] = new Neuron[8]; //hidden layer
    layers[3] = new Neuron[8]; //hidden layer
    layers[4] = new Neuron[8]; //hidden layer
    layers[5] = new Neuron[4]; //hidden layer
  ...
  public void tick () {
    if (tickNumber == 0 && runNumber != 0) { //start of new round, not first we need to reset populous
      ...
      if (newBest.reachedGoal) {
        stepsAllowed = round(newBest.ticksUsed*1.25f);
...
class Square {
  ...
  public void tickSquare () {
    ...
    if (!reachedGoal) {
      float[] dataToPassToNetwork = {position.x, position.y, 0,0,/*goal.x, goal.y,*/ velocity.x, velocity.y}; //TODO add the bombz
    ...
    if (!masterOnly || master) drawSquare(); //draws
