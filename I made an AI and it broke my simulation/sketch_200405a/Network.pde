class Network {
  ...
  Network () {
    ...
    layers[2] = new Neuron[4]; //hidden layer
  ...
  void mutateFrom (Network parentNetwork) {
    for (int layer_i = 0; layer_i < layers.length; layer_i++) {
      for (int neuron_i = 0; neuron_i < layers[layer_i].length; neuron_i++) {
        ...
        layers[layer_i][neuron_i].bias = parentNetwork.layers[layer_i][neuron_i].bias + random(-10, 10);

        for (int connect_i = 0; connect_i < layers[layer_i][neuron_i].connections.length; connect_i++) {
          layers[layer_i][neuron_i].connections[connect_i] = parentNetwork.layers[layer_i][neuron_i].connections[connect_i] + random(-10, 10);
        }
      }
    }
  }
